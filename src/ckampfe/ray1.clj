...
#_(defn reflect-ray-off-circle
    "returns reflected vector starting at the intersection point"
    [circle-centroid r start-point intersection-point]
    (let [ray-v (m/sub start-point intersection-point)
          radius-v (m/sub intersection-point circle-centroid)
        ;; intersection-normal (m/mul (m/normalise radius-v) intersection-point)
        ;; intersection-normal (m/add radius-v intersection-point)
          intersection-normal radius-v
          angle-between-normal-and-ray (v/angle (v/vec ray-v) (v/vec intersection-normal))
          rotated (rotate-wrt-point (* 2.0 angle-between-normal-and-ray) intersection-point start-point)
          rotated2 (if (>= (m/dot rotated radius-v) 0.0)
                     rotated
                     (rotate-wrt-point (* -2.0 angle-between-normal-and-ray) intersection-point start-point))
          angle-between-rotated-and-normal (v/angle (v/vec rotated2) (v/vec intersection-normal))]
      (println "angle between ray and normal" angle-between-normal-and-ray)
      (println "angle between rotated and normal" angle-between-rotated-and-normal)
      ...
      (let [is (intersections
        ...
        (cond
          ...
          (doseq [[ix iy] (take 1 (sort-by (fn [p]
            ...
            (let [[iinx iiny] (surface-normals-at-intersection [200 (/ height 2)]
                                                               circle-radius
                                                               [ix iy])
                  ...
                  [rpx rpy] (reflect-ray-off-circle [200 (/ height 2)]
                                                    circle-radius
                                                    ray-origin
                                                    [ix iy])
                  [rrpx rrpy] (m/add [ix iy] (m/mul [rpx rpy] 500))]
