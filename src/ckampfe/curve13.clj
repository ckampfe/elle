...
(defn overlapping-triangles [n points]
  (take n (iterate (fn [[x1 y1 x2 y2 x3 y3]]
                     (let [[cx cy] (centroid x1 y1 x2 y2 x3 y3)
                           scaling-factor (/ (+ (* #_(rand 0.34)0.15 (Math/sqrt 240)) 240)
                                             240)]
  ...
  #_(take n (iterate (fn [[x1 y1 x2 y2 x3 y3]]
                     (let [[cx cy] (centroid x1 y1 x2 y2 x3 y3)
...
(defn draw []
  (let [;; origin {:p1 {:x (- (/ width 2) 5)
        ...
        ;;         :p3 {:x (+ (/ width 2) 5)
        ;;              :y (+ (/ height 2) 15)}}
        origins (many 140 138 230 230 5 3)
        ...
        starting-triangles (map (fn [[x y]]
                                  {:p1 {:x (- x 10)
                                   ...
                                   :p3 {:x (+ x 64)
                                        :y (+ y 10)}})
                                origins)]
    (doseq [ps (take 30 starting-triangles)]
