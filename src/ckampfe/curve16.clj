  ...
  (take n (iterate (fn [[x1 y1 x2 y2 x3 y3 x4 y4 x5 y5 x6 y6]]
                     (let [[cx cy] (centroid x1 y1 x2 y2 x3 y3 x4 y4 x5 y5 x6 y6)
                           scaling-factor (/ (+ (* 0.45 (Math/sqrt x1)) x1)
                                             x1)]
                       (flatten [
                                 (rotate-and-scale 23 scaling-factor x1 y1 cx cy)
                                 (rotate-and-scale 30 scaling-factor x2 y2 cx cy)
                                 (rotate-and-scale 23 scaling-factor x3 y3 cx cy)
                                 (rotate-and-scale 30 scaling-factor x4 y4 cx cy)
                                 (rotate-and-scale 23 scaling-factor x5 y5 cx cy)
                                 (rotate-and-scale 30 scaling-factor x6 y6 cx cy)
...
(defn draw []
  (let [triangles (overlapping-triangles 72)]
