    ...
    (fn []
      (case (:direction @state)
        :positive (if (<= (+ (:position @state) (Math/sqrt (- max-dim (:position @state))))
                    ...
                    (swap! state update :position (fn [old] (+ old (* (Math/sqrt old)
                                                                      tick-size))))
        ...
        :negative (if (<= min-dim
                          (- (:position @state)
                             #_(Math/sqrt (- (:position @state) min-dim))
                             (* (Math/sqrt (:position @state))
                                tick-size)
...
(try (q/defsketch example
       ...
       :size [1200 800]
       ;; :renderer :svg
       ;; :output-file "moving-lines-11.svg"
